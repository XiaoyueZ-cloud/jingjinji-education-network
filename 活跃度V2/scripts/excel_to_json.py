...
def main(excel_path):
    ...
    # 指数规则：以开始年份（2023）各月原始总活跃度的算术平均值 a 作为基期，a = 100。
    base_rows = [row for row in monthly if row['month'].startswith('2023-')]
    baseline = sum(row['total'] for row in base_rows) / len(base_rows)
    monthly = [{'month': row['month'], 'categories': {name: round(value / baseline * 100, 1) for name, value in row['categories'].items()}, 'total': round(row['total'] / baseline * 100, 1)} for row in monthly]
    ...
    print(f'已写入 {len(monthly)} 个月份、{len(details)} 个事件。2023 年基期均值：{baseline:.2f}；分类核验差异：{category_errors}；总分核验差异：{total_errors}。')
