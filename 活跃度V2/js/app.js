...
const fmtIndex = value => Number(value).toFixed(1);
...
function renderChart(){
  ...
  CATEGORIES.forEach(n => series.push({name:n,type:'line',yAxisIndex:1,data:monthly.map((x,i)=>visible(x.categories[n]||0,i)),smooth:.28,symbol:'circle',symbolSize:4,lineStyle:{width:2,color:COLORS[n],shadowBlur:7,shadowColor:COLORS[n]},itemStyle:{color:COLORS[n]},z:4}));
...
function tooltip(month){ const x=monthly.find(v=>v.month===month); const max=Math.max(...CATEGORIES.map(c=>x.categories[c])); return `<b>${fmtMonth(month)}</b><br/>总活跃度：<b style="color:#ffc766">${fmtIndex(x.total)}</b><div style="margin-top:8px">${CATEGORIES.map(c=>`${c}　${fmtIndex(x.categories[c])}　<span style="display:inline-block;width:${Math.round(x.categories[c]/max*70)}px;height:6px;background:${COLORS[c]};border-radius:4px"></span>`).join('<br/>')}</div>`; }
function setMonth(index){ currentIndex=(index+monthly.length)%monthly.length; const row=monthly[currentIndex]; $('currentMonth').textContent=fmtMonth(row.month); $('timelineLabel').textContent=row.month; $('timeline').value=currentIndex; $('structureMonth').textContent=fmtMonth(row.month); $('totalScore').textContent=fmtIndex(row.total); renderBars(row); renderEvents(row.month); renderChart(); }
function renderBars(row){ const max=Math.max(...CATEGORIES.map(c=>row.categories[c])); $('categoryBars').innerHTML=CATEGORIES.map(c=>`<div class="bar-row"><span>${c}</span><div class="bar-track"><div class="bar-fill" style="width:${max?row.categories[c]/max*100:0}%;background:${COLORS[c]}"></div></div><span class="bar-value">${fmtIndex(row.categories[c])}</span></div>`).join(''); }
...
function start(){ clearInterval(timer); if(!playing||hovering)return; timer=setInterval(()=>setMonth(currentIndex+1),300); }
