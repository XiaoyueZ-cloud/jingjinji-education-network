...
async function loadData(){
  ...
  catch(e) { monthly = createDirectOpenMock(); events = createMockEvents(); }
}
...
function resetEventScroll(count){ clearInterval(eventTimer); if(count<2) return; eventTimer=setInterval(()=>{ if(hovering||!playing) return; eventOffset=(eventOffset+1)%count; $('eventList').style.transform=`translateY(-${eventOffset*105}px)`; },2700); }
